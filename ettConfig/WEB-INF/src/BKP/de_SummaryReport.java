...
public class de_SummaryReport extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response)throws ServletException, java.io.IOException
	{
		...
		try
		{
			...
			response.sendRedirect(response.encodeURL("jsp/de_summary.jsp"));
			System.out.println("query has been executed...!!!");
		}
		catch(Exception Eb)
		{
			Eb.printStackTrace();
			System.out.println("["+GetDate()+"]"+"In de_SummaryReport.java:    SQLState:  " + Eb.getMessage() );
	...
	public void GetResultData(String fromDateTime, String toDateTime,String user_circle,String HubID,ArrayList FileStatusList)
	{
		...
		{
			SelectQuery="select T1.FILE_NAME,T1.DATEVAL,T1.TIMEVAL,T1.TOTAL,T1.SUCCESS,T1.FAIL from "+TableName+" T1 , CIRCLE_CODE T2 where T1.DATEVAL >= '"+fromDateTime+"'AND T1.DATEVAL <= '"+toDateTime+"'";
		}
		else
		{
			SelectQuery="select T1.FILE_NAME,T1.DATEVAL,T1.TIMEVAL,T1.TOTAL,T1.SUCCESS,T1.FAIL from "+TableName+" T1 , CIRCLE_CODE T2 where T1.DATEVAL >= '"+fromDateTime+"'AND T1.DATEVAL <= '"+toDateTime+"'";
		...
		System.out.println("["+GetDate()+"]"+"In de_SummaryReport.java:    Query to be run is "+SelectQuery);
		try
		{
		...
		{
			System.out.println("["+GetDate()+"]"+"In de_SummaryReport.java:   Not able to connect with ORACLE");
			od.printStackTrace();
		}
		System.out.println("["+GetDate()+"]"+"In de_SummaryReport.java:    ORA Connection Created!");
		try
		{
		selectStmt = activeConnection.createStatement();

		DetailsResultSet = selectStmt.executeQuery(SelectQuery);
			de_SummaryReportBean dbean = null;
			//ArrayList FileStatusList = new ArrayList ();
			while(DetailsResultSet.next())
			{
				dbean = new de_SummaryReportBean();
			...
			System.out.println("in de_SummaryReport.java file : "+FileStatusList.size());
			selectStmt.close();
			activeConnection.close();
		}
		catch(SQLException ESQL)
		{
			System.out.println("["+GetDate()+"]"+"In de_SummaryReport.java:    SQL Error" + ESQL.getMessage());
		}
		finally
		{
			try
			...
			{
				System.out.println("["+GetDate()+"]"+"In de_SummaryReport.java:    Exception: " +ee.getMessage());
				ee.printStackTrace();
			}
		}
