...
public class CircleWiseReport extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response)throws ServletException, java.io.IOException
	{
		...
		{	
			...
			if (size == 0)
                                response.sendRedirect(response.encodeURL("jsp/message.jsp?ST=E111&redirect=circlewisereport.jsp"));
                        else
                                response.sendRedirect(response.encodeURL("jsp/circlewisereport.jsp"));
		}
		catch(Exception Eb)
		{
			Eb.printStackTrace();
			System.out.println("["+GetDate()+"]"+"In CircleWiseReport.java:    SQLState:  " + Eb.getMessage() );
...
/*		SelectQuery="select T1.DATEVAL,T1.TIMEVAL,T1.FILE_NAME,T1.TOTAL,T1.SUCCESS,T1.FAIL,T1.UP_DATE,T1.UP_TIME from registrationreport T1 where T1.DATEVAL >= '"+fromDateTime+"'AND T1.DATEVAL <= '"+toDateTime+"'AND T1.TYPE='R' order by T1.DATEVAL desc,T1.TIMEVAL desc"; */

SelectQuery="select to_char(T1.SDATETIME,'yyyy-mm-dd HH24:mi:ss') , T1.FILENAME,T1.TOTALCOUNT,T1.SUCCESSCOUNT,T1.FAILCOUNT,to_char(T1.EDATETIME,'yyyy-mm-dd HH24:mi:ss') from FILEDETAILS T1 where SUBSTR(to_char(T1.SDATETIME,'yyyy-mm-dd HH24:mi:ss'),1,10)>=SUBSTR(to_char(to_date('"+fromDateTime+"','yyyy-mm-dd HH24:mi:ss'),'yyyy-mm-dd HH24:mi:ss'),1,10) AND SUBSTR(to_char(T1.SDATETIME,'yyyy-mm-dd HH24:mi:ss'),1,10)<= SUBSTR(to_char(to_date('"+toDateTime+"','yyyy-mm-dd HH24:mi:ss'),'yyyy-mm-dd HH24:mi:ss'),1,10) AND T1.FILEACTION = 'INSERT'order by T1.SDATETIME desc";





		System.out.println("["+GetDate()+"]"+"In CircleWiseReport.java:    Query to be run is "+SelectQuery);
		try
		{
		...
		{
			System.out.println("["+GetDate()+"]"+"In CircleWiseReport.java:   Not able to connect with ORACLE");
			od.printStackTrace();
		}
		System.out.println("["+GetDate()+"]"+"In CircleWiseReport.java:    ORA Connection Created!");
		try
		{
			selectStmt = activeConnection.createStatement();
			DetailsResultSet = selectStmt.executeQuery(SelectQuery);
			CircleWiseReportBean dbean = null;
			while(DetailsResultSet.next())
			{

                                dbean = new CircleWiseReportBean();
	...
	/*			dbean = new CircleWiseReportBean();
		...
		{
			System.out.println("["+GetDate()+"]"+"In CircleWiseReport.java:    SQL Error" + ESQL.getMessage());
		}
		finally
		{
			try
			...
			{
				System.out.println("["+GetDate()+"]"+"In CircleWiseReport.java:    Exception: " +ee.getMessage());
				ee.printStackTrace();
			}
		}
		return size;
