...
public class SMS_BlockReport extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response)throws ServletException, java.io.IOException
	{
		...
		{	
			...
			if(size == 0 )
				response.sendRedirect(response.encodeURL("jsp/message.jsp?ST=E111&redirect=SMS_blockreport.jsp"));
			else
				response.sendRedirect(response.encodeURL("jsp/SMS_blockreport.jsp"));
		}
		catch(Exception Eb)
		{
			Eb.printStackTrace();
			System.out.println("["+GetDate()+"]"+"In SMS_BlockReport.java:    SQLState:  " + Eb.getMessage() );
	...
	public int GetResultData(String fromDateTime, ArrayList FileStatusList)
	{
		...
		int size			= -1;
			SelectQuery = "select FILENAME ,PATH FROM BLOCKED_REPORT WHERE DATE_CREATED ='"+fromDateTime+"' and FILE_TYPE='sms'";
		System.out.println("["+GetDate()+"]"+"In SMS_BlockReport.java:    Query to be run is "+SelectQuery);
		try
		{
		...
		{
			System.out.println("["+GetDate()+"]"+"In SMS_BlockReport.java:   Not able to connect with ORACLE");
			od.printStackTrace();
		}
		System.out.println("["+GetDate()+"]"+"In SMS_BlockReport.java:    ORA Connection Created!");
		try
		{
		...
		{
			System.out.println("["+GetDate()+"]"+"In SMS_BlockReport.java:    SQL Error" + ESQL.getMessage());
			size	= 0;
		}
		finally
		{
			...
			{
				System.out.println("["+GetDate()+"]"+"In SMS_BlockReport.java:    Exception: " +ee.getMessage());
				ee.printStackTrace();
			}
			return size;
		}
