...
public class AddShareOffer extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response)throws ServletException, java.io.IOException
	{
		...
		try
		{
			...
			{
				flag = -1;
				size = GetResultData(offerName,offerInfo,description,offerAmount,offerType,payouttype,osType,priority,status,alertText,shareText,shareMessage,callBackNotification,imageURL,actionURLAndroid,vendorNameAndroid,actionURLiOS,vendorNameiOS,shareURL,balanceType,iOSAmount,application_username);
			}
			if(size == 0 )
			{
				response.sendRedirect(response.encodeURL("jsp/message.jsp?ST=E021&redirect=shareOfferConfig.jsp"));	
			}
			else
				response.sendRedirect(response.encodeURL("jsp/shareOfferConfig.jsp"));
		}
		catch(Exception Eb)
		{
			Eb.printStackTrace();
			System.out.println("["+GetDate()+"]"+"In AddShareOffer.java:    SQLState:  " + Eb.getMessage() );
	...
	public int GetResultData(String offerName,String offerInfo,String description,String offerAmount,String offerType,String payouttype,String osType,String priority,String status,String alertText,String shareText,String shareMessage,String callBackNotification,String imageURL,String actionURLAndroid,String vendorNameAndroid,String actionURLiOS,String vendorNameiOS,String shareURL,String balanceType,String iOSAmount,String application_username)
	{
		...
		{
			System.out.println("["+GetDate()+"]"+"In AddShareOffer.java:   Not able to connect with ORACLE");
			od.printStackTrace();
		}
		System.out.println("["+GetDate()+"]"+"In AddShareOffer.java:    ORA Connection Created! and Execute Query User ["+application_username+"] Query ["+SelectQuery+"]");
		try
		{
			...
			selectStmt.setString(22,GetDate());
			selectStmt.setString(23,iOSAmount);
		...
		{
			size = 0;	
			System.out.println("["+GetDate()+"]"+"In AddShareOffer.java:    SQL Error" + ESQL.getMessage());
			return size;
		}
		finally
		{
			...
			{
				System.out.println("["+GetDate()+"]"+"In AddShareOffer.java:    Exception: " +ee.getMessage());
				ee.printStackTrace();
			}
			return size;
		}
