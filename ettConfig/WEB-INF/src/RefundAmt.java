...
public class RefundAmt extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response)throws ServletException, java.io.IOException
	{
		...
		try
		{
			...
			/*if(size == 0 )
			{
				...
				SendPushMSG.sendPost(EttId,msg);
				response.sendRedirect(response.encodeURL("jsp/message.jsp?ST=E021&redirect=refundAmt.jsp"));	
			}*/
			else
				response.sendRedirect(response.encodeURL("jsp/refundAmt.jsp"));
		}
		catch(Exception Eb)
		{
			Eb.printStackTrace();
			System.out.println("["+GetDate()+"]"+"In RefundAmt.java :    SQLState:  " + Eb.getMessage() );
			response.sendRedirect(response.encodeURL("jsp/redirectpage.jsp?ST=E016&redirect=refundAmt.jsp"));
		}
