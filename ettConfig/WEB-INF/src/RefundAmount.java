...
public class RefundAmount extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response)throws ServletException, java.io.IOException
	{
		...
		try
		{
			...
			if(flag == 0)
			{
				...
				{
					eid request.getParameter("offer_name");
					UserAccountSummary(ettid,amt,OfferName)
					sendMSG(msg,ettid);
				}
				else
				{
			...
			response.sendRedirect(response.encodeURL("jsp/RefundAmount.jsp"));
		}
		catch(Exception Eb)
		{
			Eb.printStackTrace();
			System.out.println("["+GetDate()+"]"+"In RefundAmount.java:    SQLState:  " + Eb.getMessage() );
	...
	public static void getETT_ID(ArrayList ETT_ID,String OfferName)
        ...
        public static void getOFFER(HashMap appKey)
        {
                String SelectQuery              = "";
                ...
                SelectQuery="select offerId,appKey from OfferDetails";
                try
                {
                        activeConnection  = OracleConnectionUtil.getConnection();
                }
	...
	public int UserAccountSummary(String ettid,String amt,String offerId, String offerName)
        {
                String SelectQuery              = "";
                ...
                SelectQuery="insert into UserAccountSummary(amount,createdTime,ettId,offerId,remarks,offerName) values(?,?,?,?,?,?) ";
                ...
                {
                        System.out.println("["+GetDate()+"]"+"In UpdateRedeemStatus.java:   Not able to connect with ORACLE");
                        od.printStackTrace();
                }
                System.out.println("["+GetDate()+"]"+"In UpdateRedeemStatus.java:    ORA Connection Created!");
                try
                {
                        selectStmt = activeConnection.prepareStatement(SelectQuery);
                        selectStmt.setString(1,amt);
                        selectStmt.setString(2,NewGetDate());
                        selectStmt.setString(3,ettid);
                        selectStmt.setString(4,offerId);
                        selectStmt.setString(5,offerName);
                ...
                {
                        size = 0;
                        System.out.println("["+GetDate()+"]"+"In UpdateRedeemStatus.java:    SQL Error" + ESQL.getMessage());
                        return size;
                }
                finally
                {
                        ...
                        {
                                                        System.out.println("["+GetDate()+"]"+"In UpdateRedeemStatus.java:    Exception: " +ee.getMessage());
                                ee.printStackTrace();
                        }
                        return size;
                }
