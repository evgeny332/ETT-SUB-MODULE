...
public class TimeoutHandler {
	...
	public void handler() {
		...
		for (UserRedeem userRedeem : list) {
			...
			boolean sos = false;
			if (configHolder.getProperties().getProperty("SOS_OFFER").equals("true")) {
				sos = true;
			}
			...
			if (configHolder.getProperties().getProperty("USERACTIVITYBOOSTER").equals("true") && userRedeem.getFee() == 0.0f) {
			...
			if (diffInDays < 7) {
				if (configHolder.getProperties().getProperty("FIRSTREDEEMPAY").equals("true")) {
	...
	private void rechargeBonus(User user) {
		...
		if (configHolder.getProperties().getProperty("APRIL_FOOL_OFFER").equals("true")) {
			long start = System.currentTimeMillis();
			String url = configHolder.getProperties().getProperty("APRIL_FOOL_OFFER_API").replace("#ETTID#", user.getEttId() + "").replace("#OTP#", user.getOtp() + "");
	...
	private void CheckMsisdn(User user, UserRedeem userRedeem) {
		...
		if (user.getMsisdn().equalsIgnoreCase(userRedeem.getMsisdn() + "")) {
			...
			if (!isAvailable) {
				...
				if (configHolder.getProperties().getProperty("REDEEM_SMS_ENABLE").equals("true")) {
